...
const MessagesPage = async () => {
    ...
    // sort -1 means asc order
    // populate allows to get the field 'username' in users from the 'sender' field in messages and the field 'name' from the 'property' field
        ...
        <section className="bg-blue-50">
            <div className="container m-auto py-24 max-w-6xl">
                <div className="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
                    ...
                    <div className="space-y-4">{messages.length === 0 ? <p>You have no messages</p> : messages.map(message => <MessageCard key={message._id} message={message} />)}</div>
                </div>
            </div>
        </section>
    )
